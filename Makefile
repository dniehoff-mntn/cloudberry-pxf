...
# TODO: find a way to parameterize skipping FDW builds. In CB, the major version is always 1
#       this will currently never build FDW because the underlying libraries differ
...
PXF_PACKAGE_NAME := pxf-cbdb$(GP_MAJORVERSION)-$(PXF_VERSION)-$(GP_BUILD_ARCH)
...
rpm: stage
	...
	-bb $${PWD}/build/rpmbuild/SPECS/pxf-cbdb$${GP_MAJORVERSION}.spec ;\
...
rpm-tar: rpm
	...
	set -e ;\
	PXF_RPM_FILE=$$(find build/rpmbuild/RPMS -name pxf-cbdb$${GP_MAJORVERSION}-*.rpm) ;\
...
deb: stage
	...
	PXF_MAIN_VERSION=$${PXF_VERSION//-SNAPSHOT/} ;\
	if [[ $${PXF_VERSION} == *"-SNAPSHOT" ]]; then PXF_RELEASE=SNAPSHOT; else PXF_RELEASE=1; fi ;\
	mkdir -p build/debbuild/usr/local/pxf-cbdb$${GP_MAJORVERSION} ;\
	cp -a build/stage/$${PXF_PACKAGE_NAME}/pxf/* build/debbuild/usr/local/pxf-cbdb$${GP_MAJORVERSION} ;\
	...
	mv build/debbuild.deb build/pxf-cbdb$${GP_MAJORVERSION}-$${PXF_MAIN_VERSION}-$${PXF_RELEASE}-ubuntu18.04-amd64.deb ;\
...
deb-tar: deb
	...
	set -e ;\
	PXF_DEB_FILE=$$(find build/ -name pxf-cbdb$${GP_MAJORVERSION}*.deb) ;\
